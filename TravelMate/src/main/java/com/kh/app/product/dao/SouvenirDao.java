...
public class SouvenirDao {
	...
	public List<SouvenirVo> getSouvenirList(Connection conn, PageVo pv, String searchType, String searchValue, String local) throws Exception {
		...
		if(searchType.equals("name")) {
			...
			sql = "SELECT * FROM ( SELECT ROWNUM RNUM , T.* FROM ( SELECT S.NO, S.LOCAL_NO, S.NAME, S.PRICE, S.ENROLL_DATE, S.DELETE_YN, S.COUNT, S.CONTENT, SI.TITLE, LC.NAME AS LOCAL FROM SOUVENIR S JOIN SOUVENIR_IMG SI ON SI.SOUVENIR_NO = S.NO JOIN LOCAL_CATEGORY LC ON LC.NO = S.LOCAL_NO WHERE S.DELETE_YN = 'N' AND S.NAME LIKE '%'||?||'%' AND LC.NAME LIKE '%충천남/북도%' ORDER BY NO DESC ) T ) WHERE RNUM BETWEEN ? AND ?";
		}else if(local.equals("kung")) {
			sql = "SELECT * FROM ( SELECT ROWNUM RNUM , T.* FROM ( SELECT S.NO, S.LOCAL_NO, S.NAME, S.PRICE, S.ENROLL_DATE, S.DELETE_YN, S.COUNT, S.CONTENT, SI.TITLE, LC.NAME AS LOCAL FROM SOUVENIR S JOIN SOUVENIR_IMG SI ON SI.SOUVENIR_NO = S.NO JOIN LOCAL_CATEGORY LC ON LC.NO = S.LOCAL_NO WHERE S.DELETE_YN = 'N' AND S.NAME LIKE '%'||?||'%' AND LC.NAME LIKE '%경상남/북도%' ORDER BY NO DESC ) T ) WHERE RNUM BETWEEN ? AND ?";
		}else if(local.equals("jun")) {
			sql = "SELECT * FROM ( SELECT ROWNUM RNUM , T.* FROM ( SELECT S.NO, S.LOCAL_NO, S.NAME, S.PRICE, S.ENROLL_DATE, S.DELETE_YN, S.COUNT, S.CONTENT, SI.TITLE, LC.NAME AS LOCAL FROM SOUVENIR S JOIN SOUVENIR_IMG SI ON SI.SOUVENIR_NO = S.NO JOIN LOCAL_CATEGORY LC ON LC.NO = S.LOCAL_NO WHERE S.DELETE_YN = 'N' AND S.NAME LIKE '%'||?||'%' AND LC.NAME LIKE '%전라남/북도%' ORDER BY NO DESC ) T ) WHERE RNUM BETWEEN ? AND ?";
			...
			sql = "SELECT * FROM ( SELECT ROWNUM RNUM , T.* FROM ( SELECT S.NO, S.LOCAL_NO, S.NAME, S.PRICE, S.ENROLL_DATE, S.DELETE_YN, S.COUNT, S.CONTENT, SI.TITLE, LC.NAME AS LOCAL FROM SOUVENIR S JOIN SOUVENIR_IMG SI ON SI.SOUVENIR_NO = S.NO JOIN LOCAL_CATEGORY LC ON LC.NO = S.LOCAL_NO WHERE S.DELETE_YN = 'N' AND S.NAME LIKE '%'||?||'%' AND LC.NAME LIKE '%울산%' ORDER BY NO DESC ) T ) WHERE RNUM BETWEEN ? AND ?";
			...
			sql = "SELECT * FROM ( SELECT ROWNUM RNUM , T.* FROM ( SELECT S.NO, S.LOCAL_NO, S.NAME, S.PRICE, S.ENROLL_DATE, S.DELETE_YN, S.COUNT, S.CONTENT, SI.TITLE, LC.NAME AS LOCAL FROM SOUVENIR S JOIN SOUVENIR_IMG SI ON SI.SOUVENIR_NO = S.NO JOIN LOCAL_CATEGORY LC ON LC.NO = S.LOCAL_NO WHERE S.DELETE_YN = 'N' AND S.NAME LIKE '%'||?||'%' AND LC.NAME LIKE '%광주%' ORDER BY NO DESC ) T ) WHERE RNUM BETWEEN ? AND ?";
	...
	public int order(SouvenirVo vo, Connection conn) throws Exception {
		...
		String sql = "INSERT INTO SOUVENIR_RESERVATION ( NO, SOUVENIR_NO, CNT, PRICE, NAME, PHONE, ADDRESS ) VALUES (SEQ_SOUVENIR_RESERVATION_NO.NEXTVAL, ?, ?, ?, ?, ?, ?)";
		PreparedStatement pstmt = conn.prepareStatement(sql);
	...
	public SouvenirVo selectOrder(Connection conn, String no) throws Exception {
		//SQL
		String sql = "SELECT SR.NO, S.NAME, SR.PRICE, SR.CNT, SR.NAME AS MNAME, SR.PHONE, SR.ADDRESS, SI.TITLE FROM SOUVENIR_RESERVATION SR JOIN SOUVENIR S ON S.NO = SR.SOUVENIR_NO JOIN SOUVENIR_IMG SI  ON SI.SOUVENIR_NO = S.NO WHERE S.DELETE_YN = 'N' ORDER BY NO DESC";
		PreparedStatement pstmt = conn.prepareStatement(sql);
		ResultSet rs = pstmt.executeQuery();
		//tx || rs
		SouvenirVo vo = null;
