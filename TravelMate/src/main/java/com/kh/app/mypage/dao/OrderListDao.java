...
public class OrderListDao {
	...
	public List<OrderListVo> getOrderListByNo(Connection conn, PageVo pv, String mno, String searchType, String searchValue) throws Exception {
		...
		if("category".equals(searchType) && searchValue.equals("1")) {
			...
			// sql (숙소로 검색)
			sql = "SELECT * FROM ( SELECT ROWNUM RNUM, T.* FROM ( SELECT AP.NO, AP.TYPE, AP.PAY_DATE, AR.ACCOMODATION_NO, AR.PRICE, AR.MEMBER_NO, CASE(AR.START_DATE AS DATE) AS START_DATE, CAST(AR.END_DATE AS DATE) AS END_DATE, AI.TITLE, A.NAME AS GOODS_NAME FROM ACCOMODATION_PAYMENT AP JOIN ACCOMODATION_RESERVATION AR ON (AP.ACCOMODATION_RESERVATION_CODE = AR.NO) JOIN ACCOMODATION A ON (A.NO = AR.ACCOMODATION_NO) JOIN ACCOMODATION_IMG AI ON (AI.ACCOMODATION_NO = AR.ACCOMODATION_NO) WHERE AP.CANCEL_YN = 'N' AND AR.MEMBER_NO = ? ORDER BY AP.NO DESC)T ) WHERE RNUM BETWEEN ? AND ?";
		}else if("category".equals(searchType) && searchValue.equals("3")) {
			// sql (기념품으로 검색)
			sql = "SELECT * FROM ( SELECT ROWNUM RNUM, T.* FROM ( SELECT SP.NO, SP.TYPE, SP.PAY_DATE, SR.SOUVENIR_NO, SR.PRICE, SR.MEMBER_NO, SR.ADDRESS, SR.NAME, SR.CNT ,SI.TITLE, S.NAME AS GOODS_NAME FROM SOUVENIR_PAYMENT SP JOIN SOUVENIR_RESERVATION SR ON (SP.SOUVENIR_RESERVATION_CODE = SR.NO) JOIN SOUVENIR S ON (S.NO = SR.SOUVENIR_NO) JOIN SOUVENIR_IMG SI ON (SI.SOUVENIR_NO = SR.SOUVENIR_NO) WHERE SP.CANCEL_YN = 'N' AND SR.MEMBER_NO = ? ORDER BY SP.NO DESC) T ) WHERE RNUM BETWEEN ? AND ?";
		}else {
			// 예외 던져주기
//			 throw new Exception();
			// 값이 이상하면 렌트카 목록 조회
